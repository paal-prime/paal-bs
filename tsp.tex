...
This algorithm builds target cycle from a scratch. In each step it selects next point
to append to the current cycle. Set of points from which next point is
selected is limited to only $k$ closest to the last point in cycle. This limit
seems natural and usually works well. Function for calculating estimated
fitness is doing estimated playout, it chooses randomly next points, until
$l$ last moves when it's doing exhaustive search over all possibilities
as in our tests it gave performance and speed boost.
