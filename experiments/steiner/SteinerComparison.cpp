...
void runTests(std::string outputName, Dir& resdir,
    std::vector<std::string> testsNames, std::string TESTS_DIR,
...
for (auto testName : testsNames)
  {
    ...
    auto approximation = SFApxAlgo<graph_t>(instance.get_graph(),
        instance.get_vertex_set());
    table.records[0 + loadOpts].test(approximation);

    auto MSTAVHillClimb = SFHillAlgo < graph_t,
         steiner::ActiveVerticesWalker<graph_t> > (instance.get_graph(),
             instance.get_vertex_set(), initial_solution, MSTAVIterations);
    table.records[1 + loadOpts].test(MSTAVHillClimb);

    auto CBCHillClimb = SFHillAlgo < graph_t,
         steiner::BreakCycleWalker<graph_t> > (instance.get_graph(),
             instance.get_vertex_set(), initial_solution, CBCIterations);
...
int main(int argc, char **argv)
{
  ...
  runTests("comparisonSmallerTreeTests", resdir, tree_smaller_tests,
      TREE_TESTS_DIR, 1000, 1000, true);
  runTests("comparisonSmallerForestTests", resdir, forest_smaller_tests,
      FOREST_TESTS_DIR, 1000, 1000, false);

  runTests("comparisonLargerTreeTests", resdir, tree_larger_tests,
      TREE_TESTS_DIR, 1000, 1000, true);
  runTests("comparisonLargerForestTests", resdir, forest_larger_tests,
      FOREST_TESTS_DIR, 1000, 1000, false);
