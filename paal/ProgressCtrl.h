...
namespace paal
{
  ...
  struct TimeCtrl  // implements ProgressCtrl
  {
    ...
    double progress(double)
    {
      ...
      return progress_;  // TODO: add linear interpolation if needed
    }
  };
  ...
  struct TimeAutoCtrl  // implements ProgressCtrl
  {
    TimeAutoCtrl(double _available_sec) :
        start_sec(realtime_sec()), available_sec(_available_sec), progress_(0),
        passed_it(1), granularity_it(0), checkpoint_count(0) {}
